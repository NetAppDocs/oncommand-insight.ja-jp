...
+
システム管理者がアプリケーションの信頼された証明書ストアにCA証明書を自動で取り込んでいない場合は、自己署名証明書が置き換えられても、信頼されていない証明書が原因で、Insight JavaクライアントまたはFirefoxブラウザで証明書に関する警告が引き続き生成されることがあります。組織の証明書チェーンをtrustoreにインストールすることは、追加の要件です。
